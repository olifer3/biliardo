...
void normal() {
  ...
  float theta0_deg = getValidFloat(
      "Enter initial angle (-90 < theta < 90): ", static_cast<float>(-89.9999),
      static_cast<float>(89.9999));
  ...
  sf::VertexArray rectLow =
      billiard.createThickLine(low0, low1, thickness, Brown);
...
void statistics() {
  ...
  float sigma_y0 = getSigmaWithWarning(
      "Enter standard deviation of y0 (sigma_y0): ", mu_y0, r1,
      0.7f  // 70% del massimo range utile, soglia di warning
  );
