...
void Particle::move(const Billiard& billiard, float deltaTime) {
  ...
  float expected_y_upper =
      billiard.upper_slope() * x + billiard.upper_surface_intercept();
  float expected_y_lower =
      billiard.lower_slope() * x + billiard.lower_surface_intercept();
  ...
  if (y >= expected_y_upper) {
    ...
    x = (billiard.upper_surface_intercept() - last_y + static_cast<float>(tan(angle_)) * last_x) /
        (static_cast<float>(tan(angle_)) - billiard.upper_slope());
    ...
    std::cout << "New angle = "
              << ((angle_ * 180.0f) / static_cast<float>(M_PI))
              << "° ;  (x,y) of impact = ( " << x << " , " << y << " )\n";
  }
  ...
  if (y <= expected_y_lower) {
    ...
    x = (billiard.lower_surface_intercept() - last_y + static_cast<float>(tan(angle_)) * last_x) /
        (static_cast<float>(tan(angle_)) - billiard.lower_slope());
...
void Particle::print_state(const Billiard& billiard) const {
  ...
  if (position_.x < 0) {
    x = 0;
    y = static_cast<float>(tan(angle_)) * (x - last_x) + last_y;
